...
修改前端时须同步递增三处 `?v=N`：`index.html` 资源链接、`app.js` 模块导入、`sw.js` CACHE/ASSETS。当前 v25。仅修改文档无需改版本。

## 测试

1.0.3 的 `test/yakuman.test.js` 覆盖 6,944 组役满复合场景及反例。大四喜固定 2 倍、小四喜 1 倍，互斥但不提前返回；全部役满累加后统一计算。四暗刻单骑通过实际 `winTile` 等于雀头判定，不能仅看 `waitType`。双碰荣和完成的刻子从暗刻统计中排除，暗杠仍是暗刻。`test:yaku-ui` 另验证复合役满显示与亲子家、本场支付。

1.0.2 增加 `npm run test:yaku-ui`，通过真实页面录入 11 组杯口、全带幺九与七对子复合牌形，并验证门清开关。杯口对相同顺子按不重用面子的配对数计数；七对子可拆两杯口时采用普通四面子结构，不复合两种牌型。普通规则中全带幺九要求每组含幺九，且至少有一组顺子。
